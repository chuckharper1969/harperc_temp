...
def cribl_update_destination(cribl_url, cribl_auth_token, output_id, json_output):
    ...
    cribl_uri = "%s/api/v1/system/outputs/%s" % (cribl_url, output_id)

    try:
        r = requests.patch(cribl_uri, headers=headers, json=json_output, verify=False)
    except requests.exceptions.RequestException as e:
        raise("ERROR: post request %s [%s]" % (cribl_uri, str(e)))
    
    if (r.status_code != 200):
        print("cribl_update_destination(%s, %s, %s, %s)" % (cribl_url, cribl_auth_token, output_id, json_output))
        print("PATCH /api/v1/system/outputs failed, terminating")
        print(str(r.json()))
        sys.exit()
...
def cribl_delete_destination(cribl_url, cribl_auth_token, output_id):
    ...
    if (r.status_code != 200):
        print("cribl_delete_destination(%s, %s, %s)" % (cribl_url, cribl_auth_token, output_id))
        print("DELETE /api/v1/system/outputs failed, terminating")
        print(str(r.json()))
        sys.exit()
...
def cribl_add_destination(cribl_url, cribl_auth_token, json_new_output):
    ...
    cribl_uri = "%s/api/v1/system/outputs" % cribl_url
    ...
    if (r.status_code != 200):
        print("POST /api/v1/system/outputs failed, terminating")
